...
class Keithley6487(SMU):
    def __data__(self):
        super().__data__()
        self.now = {'I(A)': None, 'V(V)': None}
    def __init__(self, address: str, name: str = "Keithley6487"):
    ...
    def insInit(self):
        ...
        self.res.write("*RST\n:FORM:ELEM ALL\nFUNC 'CURR'\nSYST:ZCH ON\nRANG 2e-9\nINIT\nSYST:ZCOR:STAT OFF\n")
        time.sleep(0.5)
        self.res.write("SYST:ZCOR:ACQ\nSYST:ZCOR ON\nCURR:RANG:AUTO ON\nSYST:ZCH OFF\n")
    def stop(self):
        pass
    # get & check
    def getNow(self):
        now_str = self.res.query(":READ?\n").split(",")
    ...
    def setOutput(self, flag: bool):
        self.res.write(f"SOUR:VOLT:STAT {flag:d}\n")
    def setSrc(self, v: float):
        self.res.write(f"SOUR:VOLT {v:f}\n")
    def setNPLC(self, n: int):
        self.res.write(f"VOLT:NPLC {n:f}\n")
